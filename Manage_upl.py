...
def remove_inter_sidechains(file):
    ...
    """
    # read DataFrame
    dt = pd.read_csv(file, sep=' ', header=None, names=['1_resn', '1_rest', '1_atom', '2_resn', '2_rest', '2_atom', 'upl'])
...
def remove_inter_sidechains_with_backbone(file):
    """Takes a .upl file and returns a Pandas Dataframe in which only backbone distances and distances between a side chain atom and a atom in the same residue are maintained.
    """
    # read Dataframe
    dt = pd.read_csv(file, sep=' ', header=None,
                     names=['1_resn', '1_rest', '1_atom', '2_resn', '2_rest', '2_atom', 'upl'])
